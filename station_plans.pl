...
use JSON;
  ...
  #@{$bld_data} = grep { $bld_data->{$_}{level} > 0 and $bld_data->{$_}->{level} <= 30 } keys %{$bld_data};
...
# Get planets
  ...
  while ($loop) {
    ...
    $resume_dt->add(days => 60);

    for my $pname (sort keys %{$ssla}) {
      $check_dt = DateTime->now;
      print "Checking $pname at ", $check_dt->hms, "\n";

      unless ($bld_data->{$pname}) {
        print "No plan for $pname, removing from list\n";
        delete $ssla->{"$pname"};
        next;
      }
      if ($ssla->{"$pname"}->{busy} and $ssla->{"$pname"}->{resume} > $check_dt) {
        $resume_dt = $ssla->{"$pname"}->{resume} if ($ssla->{"$pname"}->{resume} < $resume_dt);
        next;
      }

      my $plan = $bld_data->{$pname};
      my $output;
      my $ok = eval {
      ...
      if ($ok) {
        ...
        $resume_dt = $ssla->{"$pname"}->{resume} if ($ssla->{"$pname"}->{resume} < $resume_dt);
        print "$pname will resume at ", $resume_dt, "\n";
      }
      else {
        my $error = $@;
...
#          pop @{$bld_data};
    ...
    my $span = $resume_dt->subtract_datetime($check_dt);
    #my $span2 = $resume_dt->delta_ms($check_dt);

    #print "===", join(', ', $span2->months, $span2->days, $span2->minutes, $span2->seconds, $span2->delta_seconds), "\n";
    #
    #print 'Resuming at ', $resume_dt, '?  Using: ', $span->seconds, ' -> ',
    #  join(', ', $span->months, $span->days, $span->minutes, $span->seconds), "\n";
    my $sleep_time = $span->seconds + ($span->minutes * 60) + ($span->days * 60 * 60 * 24) + 10;

    print "Spleeping for $sleep_time seconds before next check\n";
    sleep($sleep_time);
